...
export const TableSelect = (prop) => {
  ...
  const handleForward = async(ele) => {
    try {
      await fetch(
          `https://cdlmini-6a742-default-rtdb.firebaseio.com/forwardLeave.json`,
          {
            method: "POST",
            headers: {
      ...
      <Tbody fontSize={"18px"}>
        {data.map((ele) => (
          <Tr key={ele.id}>
            ...
            <Button variant={"ghost"} onClick={()=>{handleForward(ele);StudentNotificationForward(ele)}}>
              <IoCheckmarkCircleSharp fontSize={"30px"} color="green" />
            </Button>
            <Button variant={"ghost"} onClick={()=>{handleRejected(ele);StudentNotificationRejected(ele)}}>
